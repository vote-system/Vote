...
@property (weak, nonatomic) IBOutlet UILabel *screennamePrompt;
...
- (void)viewDidLoad
{
    ...
    */
    self.textFields = [[NSArray alloc] initWithObjects:self.usrnameTextField, self.screennameTextField, self.pswTextField, self.cfmpswTextField, self.emailTextField, nil];
    self.usrnameTextField.tag = USRNAME_TAG;
    //self.screennameTextField.tag = SCREEN_NAME_TAG;
    self.pswTextField.tag = PSW_TAG;
    self.cfmpswTextField.tag = CFMPSW_TAG;
    self.emailTextField.tag = EMAIL_TAG;
    self.username_unique = NO;
    self.allCheck = [[NSMutableDictionary alloc] initWithObjectsAndKeys:INPUT_WRONG, SERVER_USERNAME,  INPUT_WRONG, SERVER_PASSWORD, INPUT_WRONG, SERVER_EMAIL, nil];
    NSLog(@"%@",self.allCheck);
...
- (IBAction)send:(id)sender {
    ...
    //发送所有注册信息进行注册
    NSString *registerURL = [[NSString alloc] initWithFormat:@"http://115.28.228.41/vote/register.php"];
    self.username_unique = NO;
    NSNumber *usernameUnique = [[NSNumber alloc] initWithBool:self.username_unique];
    NSDictionary *para = @{SERVER_USERNAME: self.usrnameTextField.text, SERVER_PASSWORD:self.pswTextField.text, SERVER_EMAIL:self.emailTextField.text, SERVER_USER_UNIQUE:usernameUnique};
    NSLog(@"URL para = %@", para);
    ...
    [manager POST:registerURL parameters:para success:^(AFHTTPRequestOperation *operation, id responseObject){
        ...
        NSInteger registerResponse = [[(NSDictionary *)responseObject objectForKey:SERVER_REGISTER_RESPONSE] integerValue];
        if (registerResponse == REGISTER_SUCCESS) {
            ...
            [successAlert show];
        } else if (registerResponse == REGISTER_ERROR) {
            UIAlertView *failureAlert = [[UIAlertView alloc] initWithTitle:@"注册失败" message:@"服务器错误，请稍后再试" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [failureAlert show];
        }
        ...
        NSLog(@"Error: %@", error);
        UIAlertView *failureAlert = [[UIAlertView alloc] initWithTitle:@"注册失败" message:@"网络错误，请稍后再试" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [failureAlert show];
    }];
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    switch (textField.tag) {
        case USRNAME_TAG:
        {
            ...
            break;
         /*
        case SCREEN_NAME_TAG:
            NSLog(@"bbb");
            break;
          */
        case PSW_TAG:
        {
            [self.allCheck setObject:INPUT_WRONG forKey:SERVER_PASSWORD];
            //提示密码长度要求
            if (string.length == 0) {
        ...
        case EMAIL_TAG:
        {
            [self.allCheck setObject:INPUT_WRONG forKey:SERVER_EMAIL];
        }
            break;
        default:
            break;
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (textField.tag == USRNAME_TAG)
    {
        ...
        NSString *registerURL = [[NSString alloc] initWithFormat:@"http://115.28.228.41/vote/register.php"];
        NSNumber *usernameUnique = [[NSNumber alloc] initWithBool:self.username_unique];
        NSDictionary *para = @{SERVER_USERNAME:self.usrnameTextField.text, SERVER_USER_UNIQUE:usernameUnique};
        NSLog(@"URL para = %@", para);
        AFHTTPRequestOperationManager *manager = [[AFHTTPRequestOperationManager alloc] init];
        manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
        manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"application/json"];
        [manager POST:registerURL parameters:para success:^(AFHTTPRequestOperation *operation, id responseObject){
            NSLog(@"operation: %@", operation);
            NSLog(@"responseObject: %@", responseObject);
            NSInteger nameUsed = [[(NSDictionary *)responseObject objectForKey:SERVER_NAME_USED] integerValue];
            if (nameUsed == NAME_NOT_USED) {
                self.usrnamePrompt.text = @"该用户名可用";
                //用户名输入正确，可以通过
                [self.allCheck setObject:INPUT_RIGHT forKey:SERVER_USERNAME];
            } else if (nameUsed == NAME_BEEN_USED) {
                self.usrnamePrompt.textColor = [UIColor redColor];
            ...
            NSLog(@"operation: %@", operation.responseString);
            NSLog(@"Error: %@", error);
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"网络错误" message:@"无法检测用户名是否存在, 请稍后再试" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alert show];

        }];
    }
    /*
    else if (textField.tag == SCREEN_NAME_TAG)
    {
    ...
    else if (textField.tag == PSW_TAG)
    {
        ...
        if ([self.pswTextField.text isEqualToString:self.cfmpswTextField.text]) {
            [self.allCheck setObject:INPUT_RIGHT forKey:SERVER_PASSWORD];
        } else {
            self.cfmpswPrompt.textColor = [UIColor redColor];
    ...
    else if (textField.tag == EMAIL_TAG)
    {
        ...
        if ([emailTest evaluateWithObject:textField.text]) {
            [self.allCheck setObject:INPUT_RIGHT forKey:SERVER_EMAIL];
            self.emailPrompt.textColor = [UIColor blackColor];
            self.emailPrompt.text = @"用于找回密码";
        } else {
